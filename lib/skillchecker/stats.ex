    ...
    query =
      from a in Attendance,
        ...
        select: %{
          ...
          last_practice_name: fragment("(ARRAY_AGG(? order by make_date(extract(year from current_date)::int, extract(month from to_date(?, 'dd Mon'))::int, extract(day from to_date(?, 'dd Mon'))::int) DESC))[1]", a.sheet_name, a.sheet_name, a.sheet_name)
        },
